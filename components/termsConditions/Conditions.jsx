const Conditions = () => {
  return (
    <section className="privacy-content terms">
      <div className="overlay pt-120 pb-120">
        <div className="container">
          <div className="row justify-content-center">
            <div className="col-lg-10">
              <div className="top-wrapper">
                <p className="lastUpdate">
                  Last updated: <span className="date">June 24, 2025</span>
                </p>
                <h4>
                  We&#39;re always looking for new ways to provide privacy for
                  our customers.
                </h4>
                <p>
                  This document is an electronically generated agreement under the provisions of Rule 3 of the Information Technology (Intermediary Guidelines and Digital Media Ethics Code) Rules, 2021, read in conjunction with the Information Technology Act, 2000. Being a digital record, it does not require any physical or digital signatures.
                </p>
                <p>
                  These Terms and Conditions (the "Terms") govern your use of the website https://www.paymanent.in (the "Website") and any related pages, APIs, features, content, or application services (including but not limited to mobile applications or mobile versions of the website), all of which are owned and operated by F6 IT Service Private Limited, the parent company of Paymanent. Throughout these Terms, references to “Company,” “we,” “us,” or “our” shall mean F6 IT Service Private Limited, and “you,” “your,” “user,” or “merchant” shall mean any person or legal entity accessing or using our platform, whether registered or unregistered.
                </p>
                <p>
                  "Applicable Laws" refer to:
                </p>
                <ul>
                  <li>
                    (i) all relevant statutes, rules, regulations, notifications, guidelines, circulars, and judicial or administrative decisions issued by central, state, or local authorities in India or having jurisdiction, and
                  </li>
                  <li>(ii) directives or notices issued by relevant financial regulators, such as the Reserve Bank of India (RBI), the National Payments Corporation of India (NPCI) (including NPCI’s published documents at https://www.npci.org.in), and other regulatory bodies as amended from time to time.</li>
                </ul>
                <p>
                  Before using the Website or any of the products or services (collectively, the “Services”) offered through Paymanent, it is important that you read and understand these Terms, along with our Privacy Policy and any applicable Special Terms of Use, thoroughly. By accessing or using the Services offered by us or through our associated banks, financial partners, technology providers, or payment networks, you acknowledge and agree to be bound by these Terms from the moment you start using the platform.
                </p>
                <p>
                  Your continued use of the Services will be considered your acceptance of any updates or changes to these Terms. We recommend visiting this page regularly to review the most current version. If you disagree with any part of these Terms or the Privacy Policy, or any amendments made later, your only option is to immediately discontinue use of the Website.
                </p>
                <p>
                  In certain cases, you may also be subject to additional terms or conditions specific to particular services you use. These shall be deemed a part of the overall Terms. <br />
                  <span className="highlight">Note:</span> The services provided via the Paymanent platform are intended solely for use within the territorial jurisdiction of <span className="highlight">India</span>.
                </p>
                <h5>
                  1. Proprietary Rights
                </h5>
                <p>
                  1.1 You acknowledge and agree that F6 IT Service Private Limited, the parent company of Paymanent, retains sole ownership of all rights, title, and interest in and to the Services, including all related intellectual property rights (whether registered or unregistered). You further understand that certain elements of the Services may be labeled as confidential and may not be disclosed without prior written permission from the Company.
                </p>
                <p>
                  1.2 Subject to your compliance with these Terms, Paymanent grants you a limited, personal, non-exclusive, non-transferable, and revocable license to access and use the Website and Services solely for lawful and personal or internal business purposes. This license does not permit you to:
                </p>
                <div className="my-disc-list">
                  <ul>
                    <li>
                      Download, reproduce, modify, create derivative works from, reverse engineer, decompile, disassemble, or attempt to derive the source code of the Services
                    </li>
                    <li>
                      Sell, distribute, sublicense, assign, lease, or otherwise transfer your rights to the Services
                    </li>
                    <li>
                      Use or reproduce any trademarks, logos, service marks, trade names, or domain names belonging to Paymanent or F6 IT Service Private Limited without prior written consent
                    </li>
                    <li>
                      Remove, obscure, or alter any proprietary notices (including copyright and trademark notices) incorporated in or accompanying the Services
                    </li>
                    <li>
                      Copy, distribute, or transmit any part of the Services without express written authorization.
                    </li>
                </ul>
                <p>
                  All rights not expressly granted to you under these Terms are reserved by the Company.
                </p>
                </div>

                <h5>
                  2. Use of the Website and Services by Users
                </h5>
                <p>
                  2.1 To access and use the Website and Services offered by Paymanent, you must be at least 18 years of age and legally capable of entering into binding contracts as per applicable laws. You are responsible for maintaining the confidentiality of your login credentials and account information. Any activity carried out under your login, whether authorized by you or not, will be your responsibility. F6 IT Service Private Limited shall not be liable for unauthorized access resulting from your failure to secure your credentials.
                </p>
                <p>
                  2.2 You agree to provide true, current, and complete information during registration and to update it as necessary. Paymanent reserves the right to suspend or terminate your access if any information is found to be false, misleading, outdated, or incomplete.
                </p>
                <p>
                  2.3 By using the Website and submitting your contact details, you consent to receive communication from Paymanent, its partners, or third-party service providers via phone, SMS, email, or messaging platforms regarding their services, offers, or updates.
                </p>
                <p>
                  2.4 You expressly authorize Paymanent, its affiliates, and third-party vendors to contact you via various modes, even if your number is registered under the Do Not Disturb (DND) registry, in accordance with TRAI regulations, for service-related or marketing purposes.
                </p>
                <p>
                  2.5 You permit Paymanent to share your information with banking/financial partners, affiliates, service providers, or third parties necessary for delivering value-added services. Opting out of marketing communication will not apply retrospectively to information already shared with third parties based on your previous consent.
                </p>
                <p>
                  2.6 You acknowledge that Paymanent may perform Know-Your-Customer (KYC) checks and other regulatory compliance measures in accordance with laws such as the PMLA, RBI guidelines, and other financial regulations applicable in India.
                </p>
                <p>
                  2.7 You agree to submit KYC documentation and any other information required for due diligence to Paymanent or any of its partners. You authorize sharing of this data with financial institutions, regulators, or enforcement agencies as required. Failure to comply may result in suspension of Services.
                </p>
                <p>
                  2.8 Paymanent reserves the right to request additional KYC or customer-related information as mandated by law or regulatory authorities. Non-compliance may result in service suspension or withholding of settlements until resolved.
                </p>
                <p>
                  2.9 Paymanent shall not be responsible for the delivery, after-sales support, customer service, payment collection, or maintenance of your goods/services. You shall indemnify and hold Paymanent harmless from any liability arising from such obligations.
                </p>
                <p>
                  2.10 You declare that neither you, your affiliates, nor your beneficial owners are Politically Exposed Persons (PEPs). If this changes, you must notify Paymanent in writing immediately.
                </p>
                <p>
                  2.11 To use our Services, you consent to sharing and processing of your personal data, including sensitive data, for verification, compliance, and service provision, as permitted by applicable law. Paymanent follows industry standards for data privacy and security, but shall not be held liable for any third-party use that occurs with your prior consent.
                </p>
                <p>
                  2.12 You shall not use the Website or Services for unlawful purposes or in violation of applicable laws. As the platform operates in India, you agree to adhere to Indian laws while using <span className="highlight">Paymanent</span>.
                </p>
                <p>
                  2.13 You are prohibited from posting or transmitting any illegal, harmful, defamatory, or unauthorized content including but not limited to content that:
                </p>
                <div className="my-disc-list">
                  <ul>
                    <li>
                      Harms minors or promotes unlawful acts.
                    </li>
                    <li>
                      Infringes on third-party IP rights.
                    </li>
                    <li>
                      Includes malware or harmful code.
                    </li>
                    <li>
                      Violates public order or national interest.
                    </li>
                    <li>
                      Involves impersonation;
                    </li>
                    <li>
                      Or otherwise violates any applicable law or regulation.
                    </li>
                </ul>
                <p>
                  2.14 You warrant that:
                </p>
                <ul>
                    <li>
                      Your business is legally established and authorized in your jurisdiction.
                    </li>
                    <li>
                      You possess full legal capacity to enter into these Terms.
                    </li>
                    <li>
                      Your acceptance of these Terms is legally binding.
                    </li>
                    <li>
                      Violates public order or national interest.
                    </li>
                    <li>
                      You will not use the Services for any virtual currency, cryptocurrency, NFTs, or related activities. Violating this clause may lead to termination of Services and legal indemnification to Paymanent.
                    </li>
                </ul>
                <p>
                  2.15 You further commit that:
                </p>
                <ul>
                    <li>
                      Your Services and transactions align with your declared and approved business activities.
                    </li>
                    <li>
                      You will not resell, assign, or allow others to use Paymanent’s Services.
                    </li>
                    <li>
                      Your use does not involve unlawful gain or fraudulent activity.
                    </li>
                    <li>
                      You have customer consent to share their information for processing, compliance, and reporting to authorities.
                    </li>
                </ul>
                </div>

                {/* <div className="safe-data">
                  <h4>
                    Your data is safe with us, we will not share any information
                    with external sources.
                  </h4>
                  <p>
                    Blandit dignissim nulla varius tristique a sed integer ut
                    tempor. Augue interdum semper bibendum amet sed. Dis in at
                    ultricies tortor sit tellus. Habitant ornare aenean maecenas
                    pretium, dui ullamcorper quis. Egestas viverra et id aliquet
                    faucibus rhoncus a. Sollicitudin nisl nulla tempor pretium
                    lorem at mauris faucibus pulvinar.
                  </p>
                  <ul>
                    <li>
                      Eget purus aenean sit risus. Arcu, aliquam eget et viverra
                      risus purus. Commodo fames tristique dui pharetra elit
                      aliquet morbi.
                    </li>
                    <li>
                      Eget consectetur risus nunc lorem sit consequat aliquet.
                      Dolor velit consectetur etiam scelerisque.
                    </li>
                    <li>
                      Integer interdum sodales scelerisque diam massa quam sit
                      quis. Sed et dui a nam pulvinar. Tristique justo, donec
                      lectus vitae, cursus ligula ridiculus lacus, tincidunt.
                    </li>
                    <li>
                      Diam dictumst faucibus dui aliquet aenean nascetur feugiat
                      leo. Etiam dignissim.
                    </li>
                    <li>
                      {" "}
                      Aliquam eget et viverra risus purus. Commodo fames
                      tristique dui pharetra elit aliquet morbi.
                    </li>
                  </ul>
                </div> */}
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Conditions;
